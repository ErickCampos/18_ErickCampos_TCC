...
\addcontentsline{toc}{chapter}{Abstract}
	...
	% --
	This work proposes a low-cost, mouth-puffing-based device, since this action
	of expelling air from the mouth does not requires so much physical effort 
	from the user. The device created is able to perform the mouse click on
	computing platforms. 
	% --
	Additionally, a driver was developed to allow the device to communicate 
	with the computer via audio	jack P2 interface in order to reduce the cost 
	of production of the device in relation to other interfaces,
	such as USB. The mouse cursor is controlled by head tracking thanks to
	eViacam software.
	% --
	The evaluation took place by comparing the proposed equipment with the dwell
	time method since this is the most used click method by
	alternative mouse control tools such as head or eye trackers.. A qualitative
